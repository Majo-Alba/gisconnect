  ...
  try {
    ...
    try {
      ...
      if (tokenList.length === 0) {
        console.log(
          "[notifyStage] No FCM tokens for recipients. Will still try Web Push."
        );
      } else {
        for (let i = 0; i < tokenList.length; i += chunkSize) {
          ...
          console.log(
            `[notifyStage] FCM ${stage}: success=${resp.successCount}, fail=${resp.failureCount}`
          );
          ...
          if (badTokens.length) {
            ...
            console.warn(
              `[notifyStage] pruned ${badTokens.length} invalid FCM tokens`
            );
          }
    ...
    try {
      ...
      console.log(
        "[notifyStage] missing webpush subscriptions:",
        missingEmails
      );

      console.log(
        "[notifyStage] webpush endpoints:",
        subs.map((s) => ({
          email: s.email,
          endpoint: s.subscription?.endpoint
            ? s.subscription.endpoint.slice(0, 80) + "..."
            : "NO_ENDPOINT",
        }))
      );
      ...
      for (const s of subs) {
        ...
        try {
          ...
          if (err?.statusCode === 410 || err?.statusCode === 404) {
            ...
            wpPruned++;
            console.warn(
              "[webpush] pruned expired subscription:",
    ...
    console.log(
      `[notifyStage] summary stage=${stage} :: FCM ok=${fcmSuccess}, FCM fail=${fcmFail}, WebPush tried=${wpTried}, WebPush pruned=${wpPruned}`
    );
  } catch (err) {
