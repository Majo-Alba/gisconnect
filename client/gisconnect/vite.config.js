...
export default defineConfig({
  plugins: [
    react(),
    VitePWA({
      // We use our own manifest in /public if you have one. If not, leave false.
      manifest: false,

      // IMPORTANT: use your own SW file
      strategies: 'injectManifest',
      srcDir: 'src',          // where your sw.js lives
      filename: 'sw.js',      // will be emitted at /sw.js

      // Dev mode: emits a temp SW so you can test locally
      devOptions: { enabled: true },

      // Options for the inject-manifest step (precache glob if you later use it)
      injectManifest: {
        globPatterns: [
      ...
      // ⚠️ Remove Workbox "workbox: {}" config — not used with InjectManifest.
      // Caching/routes should be implemented inside your own src/sw.js instead.
    })
  ],
