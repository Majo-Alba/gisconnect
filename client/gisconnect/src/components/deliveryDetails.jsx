...
export default function DeliveryDetails() { 
    ...
    const claimDelivery = useCallback(async (delivererName) => { 
        ...
        try { 
            ...
            if (status === 409) { 
                ...
                // Show a small inline error, do NOT block the screen s
                etClaimState((s) => ({ ...s, status: "error", message: msg })); 
                setClaimErrMsg(msg); 
            } 
        } 
    }, [orderId, claimedByMe]); 
    ...
    const isPickupString = typeof sRaw === "string" && sRaw.trim().toLowerCase() === "recoger en matriz"; 
    const isPickupObject = !!(sRaw && typeof sRaw === "object" && (sRaw.pickup === true || sRaw.method === "pickup")); 
    ...
    const pickupDate = pickupDetails?.date ? fmtDMY(pickupDetails.date) : ""; 
    const pickupTime = (pickupDetails?.time || "").trim(); const showBlocked = claimState.status === "blocked"; 
    ...
    return ( 
        <body className="body-BG-Gradient"> 
            ...
            {showBlocked && ( 
                <div style={{ background: "#fde047", color: "#1f2937", padding: "10px 12px", borderRadius: 8, margin: "8px 16px" }}> {claimState.message || "Este pedido ya fue tomado por otra persona para entrega."} 
            </div> 
            )} 
                        ...
                        {insureShipment === true && ( 
                            <> 
                                ...
                                <input 
                                    ...
                                    type="text" 
                                    readOnly 
                                    value={insuredAmount != null ? fmtMXN(insuredAmount) : "No disponible (falta tipo de cambio)"} title={ 
                                        insuredAmount != null 
            ...
            <div className="generateLabel-Div" style={{ display: "grid", gridTemplateColumns: "30% 70%", gap: 20 }}> 
                ...
                <button 
                    ...
                    disabled={busy || !deliveryImage || !deliveryDate || !deliverer || deliverer === "Encargado" || !claimedByMe} title={ 
                        !claimedByMe ? "Toma el pedido para continuar" : 
