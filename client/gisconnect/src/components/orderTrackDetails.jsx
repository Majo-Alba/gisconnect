...
export default function OrderTrackDetails() {
  ...
  // Two key variants: with and without a single space between peso and unidad
        ...
        Papa.parse(response.data, {
          ...
          complete: ({ data }) => {
            ...
            data.forEach((row) => {
              ...
              // presentation variants from sheet
              const peso = (row.PESO_PRODUCTO || "").toString().trim();
              const unidad = (row.UNIDAD_MEDICION || "").toString().trim();

              // Most sheets store as "PESO_PRODUCTO + UNIDAD_MEDICION" (no space), but we’ll support both
              const presNoSpace = `${peso}${unidad}`;
              const presWithSpace = `${peso} ${unidad}`.trim();

              // image column candidates (covers common variations)
              const img =
                row.IMAGE_URL ||
                row.IMAGE ||
                row.IMG_URL ||
              ...
              // Store both keys to be tolerant
  ...
  const getItemImage = (item) => {
    // Try multiple key shapes to be extra safe
    const prod = item?.product || "";
    const pres = item?.presentation || "";

    const k1 = makeKey(prod, pres); // whatever the item has
    const k2 = makeKey(prod, String(pres).replace(/\s+/g, "")); // no spaces
    const k3 = makeKey(prod, String(pres).replace(/\s*/g, " ")); // force single space (rarely needed)

    const url = imageLookup[k1] || imageLookup[k2] || imageLookup[k3];

    if (!url) {
      // Debug once per miss (won’t spam if your console filters identical lines)
  ...
  const generateInvoice = () => {
    ...
    fetch("http://localhost:4000/upload-invoice", {
      method: "POST",
      body: formData,
    })
      .then((res) => res.json())
...
// here aug 16
    const updateOrderStatus = async (id, status) => {
        try {
            await fetch(`http://localhost:4000/order/${id}/status`, {
                method: "PATCH",
  ...
  const uploadEvidence = () => {
    if (!evidenceFile || !order?._id) return alert("Seleccione una imagen válida.");

    const formData = new FormData();
    formData.append("evidenceImage", evidenceFile);
    formData.append("orderId", order._id);

    fetch("http://localhost:4000/upload-evidence", {
      method: "POST",
      body: formData,
    })
      .then((res) => res.json())
        .then(async () => {
            // 1) Optimistic UI update so the progress bar moves immediately
            setOrder((prev) => (prev ? { ...prev, orderStatus: "Evidencia Subida" } : prev));
            setEvidenceFile(null);
            alert("Evidencia de pago subida exitosamente.");

            // 2) Persist to backend (non-blocking)
            updateOrderStatus(order._id, "Evidencia Subida");
        })
      .catch((err) => console.error("Error al subir evidencia:", err));
  };
      ...
      <div className="orderTracker-LimitedScroll">
            ...
            {(order.items || []).map((item, idx) => (
              <div className="orderImageAndDets-Div" key={idx}>
                ...
                <div className="orderDetails-Div">
                  ...
                  <label className="orderDets-Label">
                    <b>Precio Unitario:</b> ${Number(item.price).toFixed(2)}{item.currency ? ` ${item.currency}` : ""}
                  </label>
                </div>
              </div>
            ))}
          ...
          {/* <div className="orderNumberAndDate-Div"> */}
            ...
            <div className="requestBill-Div">
                ...
                {!canRequestInvoice && (
                    ...
                    Esta opción se habilita <br></br>cuando el pago se <br></br>verifique.
                    </div>
                )}
            </div>
            {/* end */}
        ...
        <div className="orderTracker-UploadEvidenceDiv">
            ...
            <div className="file-upload-wrapper">
                <label htmlFor="evidenceFile" className="custom-file-upload">Elegir archivo</label>
                <input id="evidenceFile" type="file" accept="image/*" onChange={(e) => setEvidenceFile(e.target.files[0])} style={{ display: "none" }}/>
                <span className="file-selected-text">
                    {evidenceFile ? evidenceFile.name : "Ningún archivo seleccionado"}
                </span>
            </div>
            <button className="uploadPaymentEvidence-Btn" type="button" onClick={uploadEvidence}>
                Subir <br></br>Evidencia
            </button>
        </div>
        </div>
      </div>
