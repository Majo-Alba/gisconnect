...
export default function OrderNow() {
  ...
  // Remove common MX corp suffixes for better matching
  const stripCompanySuffixes = (s = "") => {
    let t = String(s);
    t = t.replace(/\bS\.?\s*A\.?\s*D?E?\s*C\.?\s*V\.?\b/gi, "");     // SA de CV (variants)
    t = t.replace(/\bS\.?\s*DE\s*R\.?\s*L\.?\s*DE\s*C\.?\s*V\.?\b/gi, ""); // S de RL de CV
    t = t.replace(/\bA\.?\s*C\.?\b/gi, "");                           // A.C.
    t = t.replace(/\bS\.?\s*C\.?\b/gi, "");                           // S.C.
  ...
  // Candidate display names to match against CREDIT_SHEET_URL.NOMBRE_CLIENTE
  const getCreditNameCandidates = () => {
    const cands = [];
    // From client sheet (legacy)
    if (clientNameFromSheet) cands.push(clientNameFromSheet);
    // From Mongo profile
    const fullName = [userProfile?.nombre, userProfile?.apellido].filter(Boolean).join(" ");
    if (fullName) cands.push(fullName);
    if (userProfile?.empresa) {
      cands.push(userProfile.empresa);
      cands.push(stripCompanySuffixes(userProfile.empresa));
    }
    // From current billing (Razón Social) if facturan
    if (currentBilling?.razonSocial) {
      cands.push(currentBilling.razonSocial);
      cands.push(stripCompanySuffixes(currentBilling.razonSocial));
    }
    // Dedup (case/space-insensitive)
    const seen = new Set();
    const uniq = [];
  ...
  useEffect(() => {
    const candidates = getCreditNameCandidates();
      ...
      .then((resp) => {
        Papa.parse(resp.data, {
          ...
          complete: ({ data }) => {
            ...
            // Build normalized candidate set (with stripped variants)
            const candNorms = new Set();
            for (const c of candidates) {
              const base = norm(c);
              if (base) candNorms.add(base);
              const stripped = norm(stripCompanySuffixes(c));
              if (stripped) candNorms.add(stripped);
            }
  
            for (const r of data) {
              const cell = norm(r?.NOMBRE_CLIENTE);
              if (!cell) continue;
              if (candNorms.has(cell)) { row = r; break; }
            }
    ...
    userProfile?.empresa,
    currentBilling?.razonSocial, // ✅ now safe, defined earlier
  ]);
  ...
  const handleDownloadAndSave = async () => {
    ...
    // Set initial order status depending on payment option
    const statusAtCreate = paymentOption === "Crédito"
    ? "Evidencia Subida"
    : "Pedido Realizado";
